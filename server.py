...
def init_db_backend():
    ...
    with db_lock, db_connection() as conn:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_room_invites_invitee_status ON room_invites(invitee_id, status, expires_at)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_room_invites_inviter_status ON room_invites(inviter_id, status, updated_at)")
...
def users_are_friends(conn, first_id, second_id):
    user_a_id, user_b_id = friendship_pair(first_id, second_id)
    row = conn.execute(
        """
        SELECT id
        FROM friendships
...
def room_invites_for_user(user_id):
    now = time.time()
    with db_lock, db_connection() as conn:
        conn.execute(
            """
        ...
        rows = conn.execute(
            """
            SELECT room_invites.id,
                   room_invites.inviter_id,
                   room_invites.room_code,
                   room_invites.created_at,
                   room_invites.expires_at,
                   users.username AS inviter_username,
                   users.display_name AS inviter_name
            FROM room_invites
            JOIN users ON users.id = room_invites.inviter_id
            WHERE room_invites.invitee_id = ?
              AND room_invites.status = 'pending'
              AND room_invites.expires_at >= ?
            ORDER BY room_invites.updated_at DESC
            LIMIT 20
            """,
            (user_id, now),
        ).fetchall()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/friends":
            self.handle_friends()
            return
        if parsed.path == "/api/room-invites":
            self.handle_room_invites()
            return
    ...
    def do_POST(self):
        ...
        try:
            ...
            elif parsed.path == "/api/room-invites/send":
                self.handle_room_invite_send()
            elif parsed.path == "/api/room-invites/action":
                self.handle_room_invite_action()
    ...
    def handle_room_invites(self):
        user = self.require_user()
        if not user:
            return
        self.send_json(200, room_invites_for_user(user["id"]))
    ...
    def handle_room_invite_send(self):
        user = self.require_user()
        if not user:
            return
        data = self.read_json()
        try:
            friend_id = int(data.get("friendId", 0))
        except (TypeError, ValueError):
            self.send_json(400, {"error": "请选择要邀请的好友"})
            return
        room_code = str(data.get("room", "")).strip().upper()
        if friend_id <= 0 or friend_id == user["id"]:
            self.send_json(400, {"error": "请选择要邀请的好友"})
            return
        if not room_code:
            self.send_json(400, {"error": "请先进入房间"})
            return

        with condition:
            room = rooms.get(room_code)
            inviter_in_room = bool(
                room and any(peer.get("userId") == user["id"] for peer in room["peers"].values())
            )
        if not inviter_in_room:
            self.send_json(403, {"error": "进入房间后才能邀请好友"})
            return

        now = time.time()
        expires_at = now + ROOM_INVITE_TTL
        with db_lock, db_connection() as conn:
            friend = conn.execute(
                "SELECT id, username, display_name FROM users WHERE id = ?",
                (friend_id,),
            ...
            if not friend:
                self.send_json(404, {"error": "没有找到这个好友"})
                return
            if not users_are_friends(conn, user["id"], friend_id):
                self.send_json(403, {"error": "只能邀请已经添加的好友"})
                return
            ...
            existing = conn.execute(
                ...
                LIMIT 1
                """,
                (user["id"], friend_id, room_code, now),
            ).fetchone()
            if existing:
                conn.execute(
                    """
                ...
                conn.execute(
                    """
                    INSERT INTO room_invites (inviter_id, invitee_id, room_code, status, created_at, updated_at, expires_at)
                    VALUES (?, ?, ?, 'pending', ?, ?, ?)
                    """,
                    (user["id"], friend_id, room_code, now, now, expires_at),
                )

        self.send_json(200, {"message": f"已邀请 {friend['display_name']} 加入房间"})

    def handle_room_invite_action(self):
        user = self.require_user()
        if not user:
            return
        data = self.read_json()
        action = str(data.get("action", "")).strip().lower()
        try:
            invite_id = int(data.get("inviteId", 0))
        except (TypeError, ValueError):
            self.send_json(400, {"error": "邀请不存在"})
            return
        if invite_id <= 0:
            self.send_json(400, {"error": "邀请不存在"})
            return
        ...
        with db_lock, db_connection() as conn:
            invite = conn.execute(
                """
                SELECT id, room_code, status, expires_at
                FROM room_invites
            ...
            if not invite:
                self.send_json(404, {"error": "没有找到这条房间邀请"})
                return
            if invite["status"] != "pending":
                self.send_json(400, {"error": "这条邀请已经处理过了"})
                return
            if invite["expires_at"] < now:
                conn.execute(
                    "UPDATE room_invites SET status = 'expired', updated_at = ? WHERE id = ?",
                    (now, invite_id),
                )
                self.send_json(400, {"error": "这条邀请已经过期"})
                return

            if action == "accept":
                conn.execute(
                    "UPDATE room_invites SET status = 'accepted', updated_at = ? WHERE id = ?",
                    (now, invite_id),
                )
                self.send_json(200, {"message": "正在加入房间", "room": invite["room_code"]})
                return

            if action == "decline":
                conn.execute(
                    "UPDATE room_invites SET status = 'declined', updated_at = ? WHERE id = ?",
                    (now, invite_id),
                )
                self.send_json(200, {"message": "已忽略邀请"})
                return

        self.send_json(400, {"error": "未知的邀请操作"})
